...
namespace
{
    ...
    using tpp_ui_menu_PopupEvCall_SettingPopup_t =
        void(__fastcall*)(void* thisPtr, const char* text, uint8_t selectFlag);

    static IsArabLanguage_t gIsArabLanguage = nullptr;
    static tpp_ui_menu_PopupEvCall_SettingPopup_t
        gOrig_tpp_ui_menu_PopupEvCall_SettingPopup = nullptr;
    ...
    static constexpr size_t kBodyTextNodeOffset = 0x168;
}
...
static void ApplyArabicLeftToRightRule(void* node)
{
    ...
    if (!SafeWriteU8(reinterpret_cast<uint8_t*>(node) + kModelNodeTextAlignOffset, TEXT_ALIGN_RIGHT))
        Log("[tpp::ui::menu::PopupEvCall::SettingPopup] Failed to write right alignment.\n");
}
...
static void RestoreOriginalAlign(void* node)
{
    ...
    if (!SafeWriteU8(reinterpret_cast<uint8_t*>(node) + kModelNodeTextAlignOffset, originalAlign))
        Log("[tpp::ui::menu::PopupEvCall::SettingPopup] Failed to restore alignment.\n");
}

/* Applies Arabic alignment to the popup body text node. Parameters: thisPtr = PopupEvCall*. */
static void ApplyArabicAlignmentToPopupBodyNode(void* thisPtr)
{
    if (!thisPtr)
        return;
    ...
    if (SafeReadPtr(base + kBodyTextNodeOffset, bodyNode) && bodyNode)
        ApplyArabicLeftToRightRule(bodyNode);
}

/* Restores original alignment for the popup body text node. Parameters: thisPtr = PopupEvCall*. */
static void RestoreAlignmentForPopupBodyNode(void* thisPtr)
{
    if (!thisPtr)
        return;
...
/* Hook for SettingPopup. Parameters: thisPtr = PopupEvCall*, text = popup body text, selectFlag = original arg. */
static void __fastcall hk_tpp_ui_menu_PopupEvCall_SettingPopup(
    void* thisPtr,
    const char* text,
    uint8_t selectFlag)
{
    if (gOrig_tpp_ui_menu_PopupEvCall_SettingPopup)
        gOrig_tpp_ui_menu_PopupEvCall_SettingPopup(thisPtr, text, selectFlag);

    if (IsArabicSafe())
        ApplyArabicAlignmentToPopupBodyNode(thisPtr);
    else
        RestoreAlignmentForPopupBodyNode(thisPtr);
}

/* Installs the popup body alignment hook. Parameters: hGame = unused, kept for project consistency. */
bool InstallPopupEvCallSettingPopupArabicLeftToRightAlignHook(HMODULE hGame)
{
    UNREFERENCED_PARAMETER(hGame);

    if (!gAddr.IsArabLanguage || !gAddr.PopupEvCall_SettingPopup)
    {
        Log("[tpp::ui::menu::PopupEvCall::SettingPopup] Missing address.\n");
        return false;
    }

    gIsArabLanguage = reinterpret_cast<IsArabLanguage_t>(gAddr.IsArabLanguage);
    gTarget = reinterpret_cast<void*>(gAddr.PopupEvCall_SettingPopup);
    ...
    if (MH_CreateHook(
        gTarget,
        &hk_tpp_ui_menu_PopupEvCall_SettingPopup,
        reinterpret_cast<LPVOID*>(&gOrig_tpp_ui_menu_PopupEvCall_SettingPopup)) != MH_OK)
    {
        Log("[tpp::ui::menu::PopupEvCall::SettingPopup] MH_CreateHook failed.\n");
        return false;
    }

    if (MH_EnableHook(gTarget) != MH_OK)
    {
        Log("[tpp::ui::menu::PopupEvCall::SettingPopup] MH_EnableHook failed.\n");
        return false;
    }

    Log("[tpp::ui::menu::PopupEvCall::SettingPopup] Arabic left->right body align hook enabled.\n");
    return true;
}

/* Removes the popup body alignment hook. Parameters: none. */
void RemovePopupEvCallSettingPopupArabicLeftToRightAlignHook()
{
    ...
    Log("[tpp::ui::menu::PopupEvCall::SettingPopup] Arabic left->right body align hook removed.\n");
}